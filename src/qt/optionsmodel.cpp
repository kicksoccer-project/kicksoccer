...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeKsocAmount"))
        settings.setValue("nAnonymizeKsocAmount", 1000);

    nObfuscationRounds = settings.value("nObfuscationRounds").toLongLong();
    nAnonymizeKsocAmount = settings.value("nAnonymizeKsocAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeKsocAmount"))
        SoftSetArg("-anonymizeksocamount", settings.value("nAnonymizeKsocAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ksoc.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeKsocAmount:
            return QVariant(nAnonymizeKsocAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case ObfuscationRounds:
            ...
            break;
        case AnonymizeKsocAmount:
            nAnonymizeKsocAmount = value.toInt();
            settings.setValue("nAnonymizeKsocAmount", nAnonymizeKsocAmount);
            emit anonymizeKsocAmountChanged(nAnonymizeKsocAmount);
            break;
        case CoinControlFeatures:
