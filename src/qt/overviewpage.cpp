...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount AvailableBalance = balance - immatureBalance;

    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, AvailableBalance, false, BitcoinUnits::separatorAlways));
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeKsocAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeKsocAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeKsocAmount = strAnonymizeKsocAmount.remove(strAnonymizeKsocAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeKsocAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeKsocAmount * COIN) nMaxToAnonymize = nAnonymizeKsocAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeKsocAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeKsocAmount));
        strAnonymizeKsocAmount = strAnonymizeKsocAmount.remove(strAnonymizeKsocAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeKsocAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
