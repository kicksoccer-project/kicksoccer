...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a KickSoccer address (e.g. %1)").arg("KME1CL2T4Z8Pkbb97aY59ZKzrzymbi5peY"));
#endif
...
{
    // return if URI is not valid or is no KSOC: URI
...
{
    // Convert ksoc:// to ksoc:
    //
    //    Cannot handle this later, because ksoc:// will cause Qt to see the part after // as host,
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "KSOC.lnk";
}
...
{
    return GetAutostartDir() / "ksoc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a KSOC.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=KSOC\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ksoc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ksoc app to startup item list
